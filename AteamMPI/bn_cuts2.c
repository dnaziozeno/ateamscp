...
/* po: sum Xi = 2.  O procedimento original gera facetas com coeficientes no */
...
/* Este procedimento constroi um novo conjunto S = M(J0(beta)), onde beta e'
   a ultima restricao gerada. Para maiores detalhes ver o Teorema 2.1 na Re-
   ferencia-1. */
void GenNewSet_S(int     *nb_cuts,
		 CutType *cut)
{
  ...
  for (i = 0; i < nb_lin; i++)
   { beta = TRUE;
     Ji = line[i].first;
     while (Ji && beta)
      { if (cut[*nb_cuts - 1].coef[Ji->col] == 1)
	 { beta = FALSE;
	   break;
	 }
	Ji = Ji->right;
